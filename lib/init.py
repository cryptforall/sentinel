...
def has_Xchange_conf():
    import config
    import io

    valid_Xchange_conf = False

    # ensure Xchange_conf exists & readable
    #
    # if not, print a message stating that Xchange Core must be installed and
    # configured, including JSONRPC access in Xchange.conf
    try:
        f = io.open(config.Xchange_conf)
        valid_Xchange_conf = True
    except IOError as e:
        print(e)

    return valid_Xchange_conf
...
def main():
    ...
    if not has_Xchange_conf():
        print("XchangeCore must be installed and configured, including JSONRPC access in Xchange.conf")
        sys.exit(1)
