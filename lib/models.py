...
class GovernanceObject(BaseModel):
    ...
    # sync Xchanged gobject list with our local relational DB backend
    @classmethod
    def sync(self, Xchanged):
        golist = Xchanged.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_Xchanged(Xchanged, item)
    ...
    @classmethod
    def import_gobject_from_Xchanged(self, Xchanged, rec):
        import decimal
        import Xchangelib
        import inflection
        ...
        # shim/Xchanged conversion
        object_hex = Xchangelib.SHIM_deserialise_from_Xchanged(object_hex)
        objects = Xchangelib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from Xchanged...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from Xchanged, with every run
        ...
        # get/create, then sync payment amounts, etc. from Xchanged - Xchanged is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(Xchanged)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from Xchanged! %s" % e)
            govobj.vote_delete(Xchanged)
            return (govobj, None)
    ...
    def vote_delete(self, Xchanged):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(Xchanged, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, Xchanged, signal, outcome):
        import Xchangelib
        ...
        output = Xchanged.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = Xchangelib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(Xchanged, signal)

    def sync_network_vote(self, Xchanged, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = Xchanged.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = XchangeD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 Xchange addr, non-multisig
            if not Xchangelib.is_valid_Xchange_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Xchange address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (Xchangelib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/XchangeDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import Xchangelib
        obj_data = Xchangelib.SHIM_serialise_for_Xchanged(self.serialise())
    ...
    def prepare(self, Xchanged):
        try:
            object_hash = Xchanged.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = XchangeD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not Xchangelib.is_valid_Xchange_address(addr, config.network):
    ...
    def hash(self):
        import Xchangelib
        return Xchangelib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = XchangeD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, Xchanged):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - Xchanged.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, Xchanged):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - Xchanged.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, Xchanged):
        now = int(time.time())
        return (self.created_at < (now - Xchanged.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, Xchanged):
        if self.is_expired(Xchanged):
            return False

        return True

    def is_deletable(self, Xchanged):
        if self.is_expired(Xchanged):
            return True
