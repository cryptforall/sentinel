...
def is_valid_Xchange_address(address, network='mainnet'):
    ...
    Xchange_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Xchange address
        return False

    if (address_version != Xchange_version):
        return False
...
# shims 'til we can fix the Xchanged side
def SHIM_serialise_for_Xchanged(sentinel_hex):
    from models import XchangeD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = XchangeD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in Xchanged
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # Xchanged expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    Xchanged_hex = serialise(obj)
    return Xchanged_hex


# shims 'til we can fix the Xchanged side
def SHIM_deserialise_from_Xchanged(Xchanged_hex):
    from models import XchangeD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(Xchanged_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('Xchange.conf')
