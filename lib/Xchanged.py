...
class XchangeDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some Xchanged methods
    ...
    @classmethod
    def from_Xchange_conf(self, Xchange_dot_conf):
        from Xchange_config import XchangeConfig
        config_text = XchangeConfig.slurp_config_file(Xchange_dot_conf)
        creds = XchangeConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = Xchangelib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = Xchangelib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = Xchangelib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_Xchanged_proto_version_with_sentinel_ping)
